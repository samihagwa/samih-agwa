...
The bootstrap endpoint is authenticated and can create only the first organization. New people create their own account with a password, then see only a waiting screen until the owner approves or rejects the durable request and chooses the exact role and sections. Request creation is explicit and idempotent, with an Auth metadata trigger as a repair path, so an account cannot silently exist outside the approval queue. Approval may send an optional one-time sign-in email, but password login never depends on it. The older email-bound invitation fallback now accepts the same email and password directly and activates the membership without a Magic Link. Suspending access is blocked while the member owns open tasks, scripts, or leads.
