...
type QueueView = "upcoming" | "archive";
...
export function PublishingWorkspace() {
  ...
  return <section className="publishing-workspace">
    ...
    {showComposer && manager ? <form key={editingSchedule?.id ?? "new-publication"} ref={composerRef} id="publishing-composer" className="panel publishing-composer" onSubmit={(event) => void savePublication(event)}><div className="section-heading"><div><p className="overline">{editingSchedule ? "تعديل آمن" : "منشور + جدول ذري"}</p><h2>{editingSchedule ? `تعديل: ${editingPost?.name ?? "المنشور"}` : "جدولة نشر جديد"}</h2></div><button className="text-button" type="button" onClick={() => { setShowComposer(false); resetComposerState(); }}>إغلاق</button></div>
      ...
      <div className="publishing-schedule-block"><div className="segmented-control"><button type="button" className={scheduleType === "once" ? "active" : ""} onClick={() => setScheduleType("once")}>مرة واحدة</button><button type="button" className={scheduleType === "weekly" ? "active" : ""} onClick={() => setScheduleType("weekly")}>أسبوعي متكرر</button></div>{scheduleType === "once" ? <label><span>موعد النشر — القاهرة</span><DateInput name="once_at" type="datetime-local" defaultValue={cairoDateTimeInput(editingSchedule?.once_at ?? null)} required /></label> : <><fieldset className="weekday-picker"><legend>أيام الأسبوع</legend>{[[1,"الاثنين"],[2,"الثلاثاء"],[3,"الأربعاء"],[4,"الخميس"],[5,"الجمعة"],[6,"السبت"],[7,"الأحد"]].map(([value,label]) => <label key={value}><input type="checkbox" name="weekdays" value={value} defaultChecked={editingSchedule ? editingSchedule.weekdays?.includes(Number(value)) : value === 1} /><span>{label}</span></label>)}</fieldset><div className="form-grid"><label><span>الوقت — القاهرة</span><input name="time_local" type="time" defaultValue={editingSchedule?.time_local?.slice(0, 5) ?? "18:00"} required /></label><label><span>تاريخ البداية</span><DateInput name="starts_on" type="date" defaultValue={editingSchedule?.starts_on ?? cairoDateInput()} required /></label><label><span>تاريخ النهاية — اختياري</span><DateInput name="ends_on" type="date" defaultValue={editingSchedule?.ends_on ?? ""} /></label><label><span>أقصى عدد مرات</span><input name="occurrence_limit" type="number" min="1" max="1000" defaultValue={editingSchedule?.occurrence_limit ?? 12} required /></label></div></>}</div>
