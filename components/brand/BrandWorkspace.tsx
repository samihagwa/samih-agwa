...
export function BrandWorkspace() {
  ...
  return <section className="brand-workspace">
    <div className="workspace-toolbar">
      ...
      <div className="toolbar-actions"><button className="icon-button" type="button" aria-label="تحديث مركز البراند" onClick={() => void refreshBrand(workspace.organization.id)}><RefreshCw size={17} /></button><Button href="/content" variant="secondary"><Route size={16} /> مصنع المحتوى</Button>{manager ? <Button type="button" onClick={() => setShowCreate((value) => !value)}><Plus size={16} /> مرجع جديد</Button> : null}</div>
    </div>
    ...
    {filteredArticles.length ? <div className="brand-article-list">{filteredArticles.map((article) => <article className={`panel brand-article-card ${article.status}`} id={`article-${article.id}`} key={article.id}>
      <header><div><p className="overline">{brandCategoryConfig[article.category].label} · v{article.version}</p><h3>{article.title}</h3><p>{article.summary}</p></div><StatusBadge tone={brandArticleStatusConfig[article.status].tone}>{brandArticleStatusConfig[article.status].label}</StatusBadge></header>
