      ...
      <section className="panel planning-calendar-panel">
        ...
        <div className="planning-calendar-list">{selectedItems.map((item) => {
          ...
          return <article className={`planning-calendar-item ${item.status === "cancelled" ? "cancelled" : ""}`} key={item.id}>
            ...
            <div className="planning-item-state"><StatusBadge tone={contentPlanItemStatusConfig[item.status].tone}>{contentPlanItemStatusConfig[item.status].label}</StatusBadge>{linked ? <a href={`/content#content-${linked.id}`}>فتح التنفيذ <ArrowUpLeft size={12} /></a> : null}</div>
