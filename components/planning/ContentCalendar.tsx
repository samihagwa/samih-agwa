...
export function ContentCalendar({ entries, canEdit, working, error, notice, details, selectedKey, undoAvailable, onSelect, onMove, onUndo, onRefresh, onCreate, onEditDraft }: Props) {
  ...
  const rangeDays = rangeEnd && rangeEnd >= anchor ? Array.from({ length: Math.min(366, Math.floor((Date.parse(rangeEnd) - Date.parse(anchor))/86400000)+1) }, (_, index) => addCalendarDays(anchor,index)) : null;
  ...
  const filtered = useMemo(() => groupCalendarEntries(entries.filter((entry) => (!platform || entry.platform === platform) && (!product || entry.product === product) && (!state || calendarState(entry).tone === state) && (!search.trim() || entry.title.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())))), [entries, platform, product, state, search]);
  ...
  const detailKey = selected?.key;
  useEffect(() => { if (detailKey) detailDialog.current?.showModal(); else detailDialog.current?.close(); }, [detailKey]);
  useEffect(() => { if (rangeOpen) rangeDialog.current?.showModal(); else rangeDialog.current?.close(); }, [rangeOpen]);
  ...
  function changeRange(direction: number) {
    if (view === "list" && rangeEnd) setRangeEnd(addCalendarDays(rangeEnd, direction * 7));
  ...
  async function finishDrop(item: Drag, target: { day: string }) {
    const entry = filtered.find((value) => value.key === item.key);
    clearDrag();
  ...
  return <section className="content-calendar-desk">
    ...
    <div className="calendar-controls"><div className="segmented-control" aria-label="طريقة عرض التقويم">{([['week','أسبوع'],['month','شهر'],['list','قائمة']] as const).map(([key,label]) => <button key={key} type="button" aria-pressed={view===key} className={view===key ? "active" : ""} onClick={() => { setView(key); clearDrag(); }}>{label}</button>)}</div>
      <div className="calendar-range"><button className="icon-button" type="button" aria-label="الفترة السابقة" onClick={() => changeRange(-1)}><ChevronRight size={18} /></button><button type="button" className="calendar-range-picker" onClick={()=>setRangeOpen(true)} aria-label="اختيار تاريخ أو فترة"><CalendarDays size={19}/>{view === "month" ? calendarDateLabel(anchor,{month:"long",year:"numeric"}) : `${calendarDateLabel(rangeDays&&view==="list"?anchor:week[0],{day:"numeric"})} – ${calendarDateLabel(rangeDays&&view==="list"?rangeEnd:week[6],{day:"numeric",month:"long",year:"numeric"})}`}</button><button className="icon-button" type="button" aria-label="الفترة التالية" onClick={() => changeRange(1)}><ChevronLeft size={18} /></button><button className="text-button" type="button" onClick={() => { setAnchor(calendarDay(new Date())); setRangeEnd(""); }}>اليوم</button></div>
    ...
    <div className="calendar-layout">
      ...
      {selected ? <dialog ref={detailDialog} className="calendar-detail calendar-detail-dialog" aria-label="تفاصيل المحتوى" onCancel={()=>onSelect(null)} onClose={()=>onSelect(null)}><header><strong>تفاصيل المحتوى</strong><button className="icon-button" type="button" aria-label="إغلاق التفاصيل" onClick={() => onSelect(null)}><X size={19}/></button></header><h2>{selected.title}</h2><p>{kindLabel(selected.kind)}</p><div className="calendar-brief"><h3>المحتوى المطلوب</h3><p>{selected.brief || "لم تتم إضافة تفاصيل بعد."}</p></div><StatusBadge tone={calendarState(selected).tone}>{calendarState(selected).label}</StatusBadge>
        {selected.source==="plan"&&canEdit&&onEditDraft?<details className="calendar-inline-edit"><summary>تعديل العنوان والتفاصيل</summary><form key={`${selected.sourceId}:${selected.title}:${selected.brief}`} onSubmit={async event=>{event.preventDefault();const form=event.currentTarget;const values=new FormData(form);if(await onEditDraft(selected,String(values.get("title")).trim(),String(values.get("brief")).trim()))form.closest("details")?.removeAttribute("open");}}><label>العنوان<input name="title" defaultValue={selected.title} required minLength={3} maxLength={180}/></label><label>التفاصيل<textarea name="brief" defaultValue={selected.brief} rows={5} minLength={5} maxLength={2000} required/></label><Button type="submit" disabled={working}>{working?"جارٍ الحفظ…":"حفظ التعديل"}</Button></form></details>:null}
    ...
    <dialog ref={rangeDialog} className="calendar-move-dialog calendar-range-dialog" aria-label="اختيار الفترة" onCancel={()=>setRangeOpen(false)} onClose={()=>setRangeOpen(false)}><header><h2>اذهب إلى التاريخ</h2><button type="button" className="icon-button" aria-label="إغلاق" onClick={()=>setRangeOpen(false)}><X size={18}/></button></header><div className="calendar-date-shortcuts">{[[0,"هذا الأسبوع"],[7,"الأسبوع القادم"],[30,"بعد شهر"]].map(([offset,title])=><button type="button" key={title} onClick={()=>{setAnchor(addCalendarDays(calendarDay(new Date()),Number(offset)));setRangeEnd("");setView("week");setRangeOpen(false);}}>{title}</button>)}</div>{rangeOpen?<DateRangeSelection start={anchor} end={rangeEnd} onApply={(start,end)=>{setAnchor(start);setRangeEnd(end);if(end)setView("list");setRangeOpen(false);}}/>:null}</dialog>
    {touchGhost?<div className="calendar-touch-ghost" style={{left:touchGhost.x,top:touchGhost.y}}>{touchGhost.title}</div>:null}
<dialog ref={dialog} className="calendar-move-dialog" onCancel={(event)=>{if(working)event.preventDefault();else setMoving(null);}} onClose={()=>setMoving(null)}><form onSubmit={async(event)=>{event.preventDefault();if(!moving||working)return;const submittedTime=String(new FormData(event.currentTarget).get("scheduledTime") ?? "");setTimeValue(submittedTime);const instant=calendarDateInstant(submittedTime,moving.scheduledAt);if(!instant){setLocalError("اختار يومًا صالحًا للنشر.");return;}if(await onMove(moving,instant))setMoving(null);}}><header><h2>تغيير الموعد</h2><button className="icon-button" type="button" aria-label="إغلاق" disabled={working} onClick={()=>setMoving(null)}><X size={18}/></button></header><strong>{moving?.title}</strong><p>{moving?(moving.members??[moving]).map(member=>contentPlatformLabel(member.platform)).join(" · "):""}</p><label>يوم النشر<DateInput name="scheduledTime" type="date" value={timeValue} onChange={(event)=>{setLocalError("");setTimeValue(event.target.value);}} required/></label>{localError||error?<p className="form-notice error" role="alert">{localError||error}</p>:null}<div className="calendar-dialog-actions"><Button type="submit" disabled={working}>{working?"جارٍ الحفظ…":"حفظ الموعد"}</Button><Button variant="ghost" type="button" disabled={working} onClick={()=>setMoving(null)}>إلغاء</Button></div>{moving?.scheduledAt?<button className="text-button" type="button" disabled={working} onClick={async()=>{if(moving&&await onMove(moving,null))setMoving(null);}}>نقل إلى محتوى بدون موعد</button>:null}</form></dialog>
  </section>;
}
