...
import { BarChart3, BookOpenCheck, Clapperboard, FilePenLine, LayoutDashboard, MessageCircleMore, Send, Settings, SquareKanban, UsersRound } from "lucide-react";
...
const contentAreaLandingBySection = {
  content: { href: "/content" },
  planning: { href: "/planning" },
  campaigns: { href: "/campaigns" },
} satisfies Record<Extract<WorkspaceSection, "content" | "planning" | "campaigns">, { href: string }>;

export function SidebarNav({ allowedSections, onNavigate }: { allowedSections: WorkspaceSection[]; onNavigate?: () => void }) {
  const pathname = usePathname();
  const allowed = new Set(allowedSections);
  const canOpenContentArea = allowed.has("content") || allowed.has("planning") || allowed.has("campaigns");
  return (
    <nav className="sidebar-nav" aria-label="التنقل الرئيسي">
      ...
      {items.filter(({ id }) => id === "content" ? canOpenContentArea : allowed.has(id)).map(({ id, href: defaultHref, label, icon: Icon }) => {
        const href = id === "content"
          ? allowed.has("content")
            ? contentAreaLandingBySection.content.href
            : allowed.has("planning")
              ? contentAreaLandingBySection.planning.href
              : contentAreaLandingBySection.campaigns.href
          : defaultHref;
        const active = id === "content"
          ? pathname.startsWith("/content") || pathname.startsWith("/planning") || pathname.startsWith("/campaigns")
