...
export function CrmCustomerWorkspace({ contactId }: { contactId: string }) {
  ...
  useEffect(() => {
    ...
    if (action !== "complete-follow-up" && action !== "convert") return;
      ...
      <div className="crm-customer-main">
        ...
        {canAct && contact.stage === "lost" ? <section className="panel crm-customer-section crm-reopen-section">
          ...
          <form className="crm-customer-result-form crm-follow-up-flow" onSubmit={(event) => void reopenCustomer(event)}>
            ...
            ] as Array<[FollowUpPreset, string]>).map(([preset, label]) => <button type="button" className={followUpPreset === preset ? "active" : ""} onClick={() => { setFollowUpPreset(preset); if (preset !== "custom") setNextFollowUpAt(followUpDate(preset)); }} key={preset}>{label}</button>)}</div><label><span>التاريخ والوقت</span><DateInput type="datetime-local" value={nextFollowUpAt} required onChange={(event) => { setFollowUpPreset("custom"); setNextFollowUpAt(event.target.value); }} /></label></div>
