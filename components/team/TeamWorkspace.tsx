...
export function TeamWorkspace() {
  ...
  async function createInvitation(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await runTeamCommand({
      ...
      role: String(form.get("role") ?? "member"), expires_in_days: Number(form.get("expires_in_days") ?? 7),
  ...
  async function updateMember(person: Person, role: Membership["role"], status: Membership["status"]) {
    ...
    const result = await runTeamCommand({ action: "update_member", organization_id: workspace.organization.id, user_id: person.id, role, status }, "تعذّر تحديث صلاحية العضو.");
    if (!result) return;
  ...
  return <section className="team-workspace">
    ...
    {!workspace.membership.onboarding_completed_at ? <section className="panel team-onboarding-panel">
      ...
      <div className="onboarding-step-grid">
        ...
        <article className={onboardingState.brand ? "complete" : ""}><BookOpenCheck size={20} /><div><strong>البراند هو المرجع</strong><p>راجع القواعد المعتمدة قبل التصميم أو المونتاج؛ تليجرام للخام والتنبيه، والموقع هو حالة الشغل.</p><a href="/brand">افتح مركز البراند <Link2 size={12} /></a></div><Button type="button" variant="secondary" disabled={working || onboardingState.brand} onClick={() => void acknowledgeOnboarding("brand")}>{onboardingState.brand ? <CheckCircle2 size={15} /> : null}{onboardingState.brand ? "تم" : "راجعت القاعدة"}</Button></article>
      </div>
    </section> : null}
    ...
    {owner ? <section className="panel team-access-panel">
      ...
      <form className="team-invite-form" onSubmit={createInvitation}>
        ...
        <label><span>الدور</span><select name="role" defaultValue="member">{manageableRoles.map((role) => <option key={role} value={role}>{roleLabel(role)}</option>)}</select></label>
        ...
        <Button type="submit" disabled={working}>{working ? <LoaderCircle className="spin" size={16} /> : <UserPlus size={16} />} إنشاء رابط فقط</Button>
      </form>
