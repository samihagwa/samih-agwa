...
- Every operational route is wrapped by an owner-approved application gate. Before an active membership is loaded, the browser renders only login, registration, recovery, or waiting surfaces and does not mount the sidebar, notifications, presence, or page workspace.
...
- Returning members sign in directly with Supabase password authentication, so normal login does not depend on email delivery. Public registration is handled by a rate-limited Edge Function that confirms the email server-side, marks the account as an owner-approval request in trusted app metadata, and explicitly creates an idempotent pending request without creating a membership. An Auth metadata-update trigger repairs interrupted or older registrations.
- Legacy owner invitations are also password-first: the high-entropy invitation remains bound to the exact email, while account creation/sign-in and invitation acceptance complete without an email or Magic Link. Activating a membership reconciles any pending request and invitation for the same account.
- A Supabase Before User Created hook is the final signup allowlist: only the fixed owner email, an unexpired pending invitation, or the trusted server-side owner-approval registration marker can create an Auth user. Authorization still comes only from database memberships and section-scoped RLS, never user metadata.
- Password recovery is enumeration-resistant and owner-assisted: the public request stores no token, the owner generates a fresh one-time Supabase recovery link, and the member sets a new password on `/reset-password`.
...
- `crm_contacts.intake_origin` distinguishes an audited manual customer creation from external/automated intake, independently of the acquisition source or product interest. The primary manual-prospect list excludes automatic indicator registrations; converted customers appear in the distinct `won` list. The cross-category “all customers” view remains intentionally comprehensive.
- A conversion from an active prospect requires an explicitly entered purchased service. The existing version-fenced CRM activity command records it in the customer history, closes the linked follow-up, and moves the customer to `won` atomically. Directory shortcuts only open that governed customer-file flow.
