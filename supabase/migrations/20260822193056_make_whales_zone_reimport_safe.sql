...
begin
  ...
  loop
    ...
    external_id := coalesce(nullif(trim(input_row->>'external_id'), ''), 'sheet-row-' || (row_number + 1)::text);
    registered_at := null;
    begin
      ...
      insert into public.crm_import_rows (
        ...
        batch_id, target_organization_id, 'google_sheet_whales_zone', external_id,
    ...
    exception when others then
      insert into public.crm_import_rows (
        ...
        coalesce(nullif(external_id, ''), 'sheet-row-' || (row_number + 1)::text),
